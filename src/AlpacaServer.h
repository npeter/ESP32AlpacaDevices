...
private:
    ...
    char _mng_server_name[64] = "empty";
    char _mng_manufacture[64] = "DIY by @BigPet";
    char _mng_manufacture_version[16] = "V1.0";
    char _mng_location[64] = "Germany/Bavaria";
...
public:
    AlpacaServer(const char *mng_server_name,
                 const char *mng_manufacture,
                 const char *mng_manufacture_version,
                 const char *mng_location);

    void Begin(uint16_t udp_port = kAlpacaUdpPort, uint16_t tcp_port = kAlpacaTcpPort);
    void Loop();
